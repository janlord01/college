    ...
    <div class="w-full sm:px-6">
        ...
        <section class="flex flex-col break-words bg-white sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg content-header">
            ...
            <header class="font-semibold bg-gray-200 text-gray-700  py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md relative ">
                <p class="header-title">Roles & Permissions</p>
                    ...
                    <table class="min-w-full w-full info-table table-auto border-collapse md:table mb-5">
                        <thead class="block md:table-header-group">
                            <tr class="border border-grey-300 md:border-none sm:hidden block md:table-row absolute -top-full md:top-auto -left-full md:left-auto md:relative tabletr">
                                <th class="bg-gray-300 p-2 text-black font-bold md:border md:border-grey-700 text-sm text-left sm:hidden block md:table-cell">Roles</th>
                                <th class="bg-gray-300 p-2 text-black font-bold md:border md:border-grey-700 text-sm text-left sm:hidden block md:table-cell ">Permissions</th>
                                ...
                                <tr class="border border-grey-500 md:border-none block md:table-row">
                                    ...
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
                                        <div class="flex items-center text-sm">
                                            <div>
                                                <p class="font-semibold text-sm sm:inline block text-black pr-1">{{ $role->name }}</p>
                                            </div>
                                            </div>
                                    </td>
                                    <td class="p-2 block md:border md:border-grey-500 text-left md:table-cell ">
                                        <span class="inline-block md:hidden font-bold pr-1 ">Permissions </span>
                                        @foreach ($role->getAllPermissions() as $key => $permission)
                                                <label class="inline-flex items-center justify-center px-2 py-1 text-xs mb-1 font-bold leading-none text-red-100 bg-blue-600 rounded-full">{{ $permission->name }}</label>
                                        @endforeach
                                    </td>
                                    ...
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
                                        ...
                                        {{-- <a class="bg-gray-500 text-center block text-white font-bold py-1 px-2 border border-gray-500 rounded mb-1">Edit</a> --}}
                                        @else
                                        <a href="#" alt="Edit Role">
                                        ...
                                        @if ($role->name == 'student' || $role->name == 'scanner' || $role->name == 'parent' || $role->name == 'teacher' || $role->name == 'super-admin'  || $role->name == 'registrar' || $role->name == 'cashier' )
                                        
                                        {{-- <a class="bg-gray-500 block  text-white font-bold py-1 px-2 border border-gray-500 rounded " disabled >Delete</a> --}}
                                            
                                        @else
                                        <a href="" alt="Delete Roles">
                                            <button class="text-black hover:text-white font-bold p-1 rounded bg-gray-300 hover:bg-gray-500">
