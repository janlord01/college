    ...
    <div class="w-full sm:px-6">
        ...
        <section class="flex flex-col break-words bg-white sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg content-header">
            ...
            <header class="font-semibold bg-gray-200 text-gray-700  py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md relative ">
                <p class="header-title">Requirements List</p>
                
                <a href="{{ route('setting.requirement.create') }}" alt="add new requirement"> 
                                ...
                                <tr class="border border-grey-500 md:border-none block md:table-row">
                                    ...
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
                                        <div class="flex items-center text-sm">
                                            <div>
                                                <p class="font-semibold text-sm sm:inline block text-black pr-1">{{ $requirement->title }}</p>
                                            </div>
                                            </div>
                                    </td>
                                    <td class="p-2 block md:border md:border-grey-500 text-left md:table-cell ">
                                        @if($requirement->status == 'Active')
                                        <span class="inline-block md:hidden font-bold pr-1 ">Status: </span><span class="bg-green-500 p-2 rounded text-sm border border-green-500 text-white">{{ $requirement->status }}</span>
                                        @else
                                        <span class="inline-block md:hidden font-bold pr-1 ">Status: </span><span class="bg-red-500 p-2 rounded text-sm border border-red-500 text-white">{{ $requirement->status }}</span>
                                        @endif
                                    </td>
                                    <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
                                        <span class="inline-block md:hidden font-bold">Actions</span>
                                        <a href="{{ route('setting.requirement.edit', $requirement->id) }}" alt="Edit Requirements">
                                        ...
                                        <form class="inline" method="POST"
                                            action="{{ route('setting.requirement.activate', $requirement->id) }}" enctype="multipart/form-data">
                                            @csrf
                                            @method('PATCH')
